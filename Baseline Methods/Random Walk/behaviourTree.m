...
%     if dist < 0.8
%         state = 1;
%     elseif dist <= 1 && agent(index_agent).state == 1
%         state = 1;
%     else
%         if agent(index_agent).chargeBool == 1 && agent(index_agent).fuel < 100
%             state = 4;
%         else
%             if agent(index_agent).fuel < threshold1
%                 if (depot.inUse == 0) 
...
%                     end
%                 else
%                     if agent(index_agent).fuel < thresholdCritic
%                         if sqrt((agent(index_agent).posX-depot.posX)^2+(agent(index_agent).posY-depot.posY)^2) < distThresh
%                             state = 3;
%                         else
%                             state = 2;
