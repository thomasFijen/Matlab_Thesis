...
% This function is called to initialise the mission space to random cell
% age values
...
function [MS,X,Y] = initEnviron_Rand(grid)
...
temp = 1*ones(grid.bredth/grid.res,grid.width/grid.res);
...
for i = 1:grid.width/grid.res
    for j=1:grid.bredth/grid.res
        if temp(j,i) ~= -1
            temp(j,i) = randi(100,1);
        end
    end
end
% -- Assigning outputs
