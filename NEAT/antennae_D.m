...
function [ ant1, ant2, ant3, ant4, ant5, ant6, ant7, ant8,ant1_cell, ant2_cell, ant3_cell, ant4_cell, ant5_cell, ant6_cell, ant7_cell, ant8_cell ] = antennae_D( agent,MS,grid,index )
% function [ant1_cell, ant2_cell, ant3_cell, ant4_cell, ant5_cell, ant6_cell, ant7_cell, ant8_cell ] = antennae_D( agent,MS,grid,index )
% antennae_D        This function adds sensors to the model. These sensors
...
%     subsetMS = MS(indexY_s:indexY_e,indexX_s:indexX_e);
    ...
    ant1_cell = 100-MS(y_index,x_index);
    while i < indexY_e && ant1 < agent(index).sensorRange
        i=i+1;
        if MS(i,x_index) ~= -1
            ant1 = ant1+grid.res;
            ant1_cell = 100-MS(i,x_index);
%             ant1_cell = max(ant1_cell,100-MS(i,x_index));
        else
            break;
        end
    ...
    ant3_cell = 100-MS(y_index,x_index);
    while i < indexX_e && ant3 < agent(index).sensorRange
        i=i+1;
        if MS(y_index,i) ~= -1
            ant3 = ant3 + grid.res;
            ant3_cell = 100-MS(y_index,i);
%             ant3_cell = max(ant3_cell,100-MS(y_index,i));
        else
            break;
        end
    ...
    ant5_cell = 100-MS(y_index,x_index);
    while i > indexY_s && ant5 < agent(index).sensorRange
        i=i-1;
        if MS(i,x_index) ~= -1
            ant5 = ant5 + grid.res;
            ant5_cell = 100-MS(i,x_index);
%             ant5_cell = max(ant5_cell,100-MS(i,x_index));
        else
            break;
        end
    ...
    ant7_cell = 100-MS(y_index,x_index);
    while i > indexX_s && ant7 < agent(index).sensorRange
        i=i-1;
        if MS(y_index,i) ~= -1
            ant7 = ant7 + grid.res;
            ant7_cell = 100-MS(y_index,i);
%             ant7_cell = max(ant7_cell,100-MS(y_index,i));
        else
            break;
        end
    ...
    ant2_cell = 100-MS(y_index,x_index);
    while x < indexX_e && y < indexY_e && ant2 < agent(index).sensorRange
        x=x+1;
        y=y+1;
        if MS(y,x) ~= -1
            ant2 = ant2 + grid.res/cos(pi/4);
            ant2_cell = 100-MS(y,x);
%             ant2_cell = max(ant2_cell,100-MS(y,x));
        else
            break;
        end
    ...
    ant4_cell = 100-MS(y_index,x_index);
    while x < indexX_e && y > indexY_s && ant4 < agent(index).sensorRange
        x=x+1;
        y=y-1;
        if MS(y,x) ~= -1
            ant4 = ant4 + grid.res/cos(pi/4);
            ant4_cell = 100-MS(y,x);
%             ant4_cell = max(ant4_cell,100-MS(y,x));
        else
            break;
        end
    ...
    ant6_cell = 100-MS(y_index,x_index);
    while x > indexX_s && y > indexY_s && ant6 < agent(index).sensorRange
        x=x-1;
        y=y-1;
        if MS(y,x) ~= -1
            ant6 = ant6 + grid.res/cos(pi/4);
            ant6_cell = 100-MS(y,x);
%             ant6_cell = max(ant6_cell,100-MS(y,x));
        else
            break;
        end
    ...
    ant8_cell = 100-MS(y_index,x_index);
    while x > indexX_s && y < indexY_e && ant8 < agent(index).sensorRange
        x=x-1;
        y=y+1;
        if MS(y,x) ~= -1
            ant8 = ant8 + grid.res/cos(pi/4);
            ant8_cell = 100-MS(y,x);
%             ant8_cell = max(ant8_cell,100-MS(y,x));
        else
            break;
        end
...
ant1 = ant1/agent(index).sensorRange;
ant2 = ant2/agent(index).sensorRange;
ant3 = ant3/agent(index).sensorRange;
ant4 = ant4/agent(index).sensorRange;
ant5 = ant5/agent(index).sensorRange;
ant6 = ant6/agent(index).sensorRange;
ant7 = ant7/agent(index).sensorRange;
ant8 = ant8/agent(index).sensorRange;
